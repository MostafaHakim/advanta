...
export default function TeamDashboardPage() {
  ...
  const stats = {
    ...
    featured: team.filter((m) => m.featured).length,
    withEmail: team.filter((m) => m.email).length,
    withSocial: team.filter((m) => Object.keys(m.social || {}).length > 0)
      .length,
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="space-y-4 md:space-y-6">
        {/* Header Section - মোবাইল ফ্রেন্ডলি */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link
            href="/admin/dashboard/team/add"
            className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all text-sm md:text-base w-full sm:w-auto"
          >
        ...
        {/* Stats Cards - ফুল রেস্পন্সিভ গ্রিড */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white rounded-xl p-4 md:p-5 shadow-lg border border-gray-200"
          >
            <div className="flex items-center justify-between">
              <div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs md:text-sm">Featured</p>
                <p className="text-xl md:text-3xl font-bold text-gray-900">
                  {stats.featured}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs md:text-sm">With Email</p>
                <p className="text-xl md:text-3xl font-bold text-gray-900">
                  {stats.withEmail}
                </p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500 text-xs md:text-sm">Social</p>
                <p className="text-xl md:text-3xl font-bold text-gray-900">
                  {stats.withSocial}
                </p>
              </div>
        ...
        {/* Filters and Search - মোবাইলে কোলাপসিবল */}
        <div className="bg-white rounded-xl md:rounded-2xl shadow-lg border border-gray-200">
          ...
          {/* ফিল্টার সেকশন - মোবাইলে টগল করা যাবে */}
          <div
            className={`${showFilters || !isMobile ? "block" : "hidden"} p-4 md:p-6`}
          >
              ...
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full pl-10 md:pl-12 pr-4 py-2.5 md:py-3 text-sm md:text-base border border-gray-300 rounded-lg md:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                  />
                </div>
              </div>

              {/* ফিল্টার বাটন এবং অপশন */}
              <div className="flex flex-wrap gap-2 md:gap-3">
                {/* ভিউ মোড টগল - ডেস্কটপে দেখাবে */}
                {!isMobile && (
                  <div className="flex border border-gray-300 rounded-lg md:rounded-xl overflow-hidden">
                    <button
                ...
                <div className="relative">
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value as any)}
                    className="appearance-none pl-3 md:pl-4 pr-8 md:pr-10 py-2.5 md:py-3 text-sm md:text-base border border-gray-300 rounded-lg md:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white"
                  >
                ...
                <button
                  onClick={() => setFilterFeatured(!filterFeatured)}
                  className={`flex items-center gap-1 md:gap-2 px-3 md:px-5 py-2.5 md:py-3 rounded-lg md:rounded-xl border transition-all text-sm md:text-base ${
                    filterFeatured
                      ? "bg-amber-50 border-amber-200 text-amber-700"
                      : "border-gray-300 hover:bg-gray-50 text-gray-700"
                  }`}
                >
                  <Star
                    className={`w-4 h-4 ${filterFeatured ? "fill-amber-500 text-amber-500" : ""}`}
                  />
                ...
                <button
                  onClick={fetchTeamData}
                  className="p-2.5 md:p-3 border border-gray-300 rounded-lg md:rounded-xl hover:bg-gray-50 transition-colors text-gray-700"
                >
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            className="p-3 md:p-4 bg-red-50 border border-red-200 rounded-lg md:rounded-xl flex items-start gap-3"
          >
            ...
            <div className="flex-1">
              ...
              <button
                onClick={fetchTeamData}
                className="mt-2 text-xs md:text-sm text-red-700 hover:text-red-900 font-medium"
              >
                Try again
              </button>
            </div>
        ...
        {/* Team Display - Grid/List View */}
        {filteredTeam.length === 0 ? (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-12 md:py-16 bg-white rounded-xl md:rounded-2xl shadow-lg border border-gray-200 px-4"
          >
            <div className="w-16 h-16 md:w-24 md:h-24 mx-auto mb-4 md:mb-6 rounded-full bg-gray-100 flex items-center justify-center">
            ...
            <Link
              href="/admin/dashboard/team/add"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 md:px-6 py-2.5 md:py-3 rounded-lg font-medium transition-colors text-sm md:text-base"
            >
            ...
            <AnimatePresence>
              {filteredTeam.map((member, index) => (
                <motion.div
                  ...
                  transition={{ delay: index * 0.05 }}
                  layout
                  className="bg-white rounded-xl md:rounded-2xl shadow-lg overflow-hidden border border-gray-200 hover:shadow-xl transition-all duration-300"
                >
                  {/* Member Card - মোবাইলের জন্য অপ্টিমাইজড */}
                  <div className="relative h-36 sm:h-40 md:h-48 bg-gradient-to-br from-gray-200 to-gray-300">
                    ...
                    {member.featured && (
                      <div className="absolute top-2 left-2 md:top-4 md:left-4">
                        <span className="inline-flex items-center gap-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs font-bold shadow-lg">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 md:top-4 md:right-4 w-7 h-7 md:w-8 md:h-8 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center hover:bg-white transition-colors shadow-md"
                    >
                      <MoreVertical className="w-3 h-3 md:w-4 md:h-4 text-gray-700" />
                    </button>
                  </div>
                  ...
                  <div className="p-3 md:p-5">
                    <div className="mb-2 md:mb-3">
                      ...
                      <p className="text-xs md:text-sm text-blue-600 font-medium truncate">
                        {member.position}
                      </p>
                    </div>
                    ...
                    {/* Expertise Tags - মোবাইলে কম দেখাবে */}
                      ...
                      <div className="flex flex-wrap gap-1 md:gap-2">
                        {member.expertise
                          ...
                          .map((skill, i) => (
                            <span
                              key={i}
                              className="px-2 md:px-3 py-0.5 md:py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-medium"
                            >
                              {skill}
                            </span>
                          ))}
                        {member.expertise.length > (isMobile ? 2 : 3) && (
                          <span className="px-2 md:px-3 py-0.5 md:py-1 bg-gray-100 text-gray-600 rounded-full text-xs">
                            +{member.expertise.length - (isMobile ? 2 : 3)}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Action Buttons - মোবাইলে স্ট্যাক */}
                    <div className="flex flex-col sm:flex-row gap-2 pt-2 md:pt-4 border-t border-gray-100">
                      <Link
                        href={`/admin/dashboard/team/${member._id}/edit`}
                        className="flex-1 flex items-center justify-center gap-1 md:gap-2 py-1.5 md:py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-lg font-medium transition-colors text-xs md:text-sm"
                      >
                      ...
                      <button
                        onClick={() => setDeleteConfirm(member._id)}
                        className="flex-1 flex items-center justify-center gap-1 md:gap-2 py-1.5 md:py-2.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium transition-colors text-xs md:text-sm"
                      >
                  ...
                  {/* Quick View - মোবাইলে ছোট করে দেখাবে */}
                  {selectedMember?._id === member._id && (
                    <motion.div
                      initial={{ opacity: 0, height: 0 }}
                      animate={{ opacity: 1, height: "auto" }}
                      exit={{ opacity: 0, height: 0 }}
                      className="border-t border-gray-100 p-3 md:p-5 bg-gray-50"
                    >
                      <div className="space-y-2 md:space-y-3">
                        ...
                        <div className="flex gap-3 md:gap-4 pt-2">
                          ...
                          {member.social?.twitter && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-blue-400 hover:text-blue-600 text-xs md:text-sm"
                            >
                              Twitter
                            </a>
                          )}
                        ...
                        <Link
                          ...
                          className="w-full flex items-center justify-center gap-2 py-1.5 md:py-2 text-xs md:text-sm text-gray-600 hover:text-gray-900 font-medium"
                        >
        ...
        {/* Delete Confirmation Modal - রেস্পন্সিভ */}
        <AnimatePresence>
          {deleteConfirm && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1 }}
                exit={{ opacity: 0, scale: 0.9 }}
                className="bg-white rounded-xl md:rounded-2xl p-5 md:p-6 max-w-md w-full mx-4 shadow-2xl"
              >
                <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 md:mb-4 rounded-full bg-red-100 flex items-center justify-center">
                ...
                <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
                  <button
                    onClick={() => setDeleteConfirm(null)}
                    className="flex-1 py-2.5 md:py-3 border border-gray-300 rounded-lg md:rounded-xl font-medium text-gray-700 hover:bg-gray-50 transition-colors text-sm md:text-base order-2 sm:order-1"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => handleDelete(deleteConfirm)}
                    className="flex-1 py-2.5 md:py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg md:rounded-xl font-medium transition-all text-sm md:text-base order-1 sm:order-2"
                  >
                    Delete Member
                  </button>
                </div>

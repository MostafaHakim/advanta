...
const TeamSection = () => {
  ...
  if (loading) {
    return (
      <section className="relative py-16 lg:py-24 overflow-hidden bg-[#F5F2EB]">
        <div className="container-custom relative z-10">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center">
              <div className="inline-block animate-spin rounded-full h-12 w-12 border-2 border-[#2A5C6E] border-t-transparent mb-4"></div>
              <h3 className="text-lg font-medium text-[#2A5C6E] mb-2">
                Loading Team
              </h3>
              <p className="text-[#5E6B6E]">Please wait a moment...</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-16 lg:py-24 overflow-hidden bg-[#F5F2EB]">
        <div className="container-custom relative z-10">
          <div className="text-center max-w-md mx-auto">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[#F8E1E1] flex items-center justify-center">
              <span className="text-2xl font-medium text-[#C45A5A]">!</span>
            </div>
            <h3 className="text-xl font-medium text-[#2A5C6E] mb-2">
              Unable to Load Team
            </h3>
            <p className="text-[#5E6B6E] mb-4">{error}</p>
            <button
              onClick={fetchTeamMembers}
              className="px-5 py-2.5 bg-[#2A5C6E] hover:bg-[#1E4554] text-white rounded-lg transition-colors text-sm font-medium"
            >
              Try Again
            </button>
          </div>
  ...
  return (
    <section className="relative py-16 lg:py-24 overflow-hidden bg-[#F5F2EB]">
      ...
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_30%_30%,#2A5C6E_0%,transparent_50%)]" />
        <div className="absolute bottom-0 right-0 w-full h-full bg-[radial-gradient(circle_at_70%_70%,#C45A5A_0%,transparent_50%)]" />
      </div>
      ...
      <div className="container-custom relative z-10">
        ...
        >
          <motion.div
            initial={{ scale: 0.95 }}
            animate={{ scale: 1 }}
            className="inline-flex items-center gap-2 px-4 py-2 bg-[#E8E2D9] rounded-full mb-4"
          >
            <Users className="w-4 h-4 text-[#2A5C6E]" />
            <span className="text-xs font-medium text-[#2A5C6E] tracking-wider uppercase">
              Our Team
            </span>
          </motion.div>

          <h2 className="text-3xl md:text-4xl font-light mb-3 text-[#2A5C6E]">
            Meet the <span className="font-medium">Minds</span>
          </h2>

          <p className="text-base text-[#5E6B6E] max-w-2xl mx-auto">
            A collective of creative thinkers and problem solvers
          </p>
        </motion.div>

        {/* Team Grid - All Cards Equal Size */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 lg:gap-6">
          {teamMembers.map((member, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="bg-white rounded-xl overflow-hidden border border-[#E8E2D9] hover:border-[#2A5C6E] transition-all duration-300 shadow-sm hover:shadow-md">
                ...
                <div className="relative aspect-square overflow-hidden bg-[#F5F2EB]">
                  <Image
                    ...
                    onError={(e) => {
                      e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name)}&background=2A5C6E&color=F5F2EB&size=400`;
                    }}
                  />

                  {/* Simple Overlay with Position */}
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#2A5C6E]/90 to-transparent p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                ...
                <div className="p-4">
                  <div className="mb-2">
                    <h3 className="font-medium text-[#2A5C6E] text-base mb-1">
                      {member.name}
                    </h3>
                    <p className="text-xs text-[#8A9A9E] line-clamp-2">
                      {member.bio}
                    </p>
                  </div>

                  {/* Expertise Tags - Simple */}
                  {member.expertise && member.expertise.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {member.expertise.slice(0, 2).map((skill) => (
                        <span
                          key={skill}
                          className="px-2 py-1 bg-[#F5F2EB] text-[#2A5C6E] rounded text-[10px] font-medium"
                        >
                          {skill}
                        </span>
                      ))}
                      {member.expertise.length > 2 && (
                        <span className="px-2 py-1 bg-[#F5F2EB] text-[#2A5C6E] rounded text-[10px] font-medium">
                          +{member.expertise.length - 2}
                        </span>
                      )}
                    </div>
                  ...
                  {/* Simple Stats Row - If available */}
                  {member.stats && member.stats.length > 0 && (
                    <div className="flex items-center gap-3 mb-3 text-[10px] text-[#5E6B6E]">
                      {member.stats.slice(0, 2).map((stat, idx) => (
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-[#E8E2D9]">
                    {member.social?.linkedin && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-1.5 text-[#8A9A9E] hover:text-[#2A5C6E] transition-colors"
                        aria-label="LinkedIn"
                      >

      ...
      <div ref={ref} className="container-custom relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            ...
            >
              {/* Icon */}
              <div className="mx-auto mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow-lg transition-shadow duration-300 group-hover:shadow-xl">
                <div className="text-blue-600">{stat.icon}</div>
              </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 opacity-70 md:gap-12">
            {["Brand A", "Brand B", "Brand C", "Brand D", "Brand E"].map(
              (brand) => (
                <span
                  key={brand}
                  className="cursor-pointer text-lg font-semibold text-white transition-colors hover:text-blue-300"
                >
                  {brand}
                </span>
              )
